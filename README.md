...
- **借入 / 借出**：分段切换列表（总额卡 + 借款时间），借出与借入均为独立账户，还款用转账即减欠款
- **账户详情页**：点账户进入 —— 账户卡（更多菜单修改/删除、✏️ 余额调整）+ **按月折叠的收支流水**：月/日 流出流入汇总、每笔之后的账户余额、转账按视角显示 ±方向、「账户创建」记录可编辑时间与金额，右上角「记一笔」直达本账户记账
- **账单详情页**：点流水进入 —— 编辑 / 删除 / 退款 / 存为模板快捷操作；**每行均可独立修改**：类型（可切换支出/收入分类）、账本、时间、金额（数字键盘）、账户、备注、标签、报销状态、退款关联
- **余额调整计算器**：带入当前余额、清除按钮、+−×÷ 四则运算键盘，可选「将增减金额记为收支」
- **账户开关**：`计入总资产`（关闭后不计入净资产与汇总，显示「不计入」徽标）、`记账时可被选择`（关闭后不出现在记账/转账/周期账户列表）
- **隐私模式**：👁 一键隐藏全部金额（¥✱✱✱✱）
- 数据自主：JSON 导出备份 / 导入恢复 / 载入演示数据 / 清空数据重新开始
...
```bash
npm install
npm run dev        # 本机 http://127.0.0.1:5173，同时监听局域网（手机可访问）
```
...
npm run preview    # 本机 http://127.0.0.1:4173，同时监听局域网
```

> 首次打开为全新空数据（仅内置「我的账本」与默认收支分类），并自动弹出使用引导介绍各功能模块；顶栏「?」可随时再次打开引导。想先体验完整功能，可在「资产 → 数据管理」一键载入演示数据；「清空数据」可随时恢复到初始状态。
...
3. （可选）Safari「分享 → 添加到主屏幕」，即可像原生 App 一样从桌面图标全屏启动

本项目已具备 PWA 基础能力：

- Web App Manifest（`public/manifest.webmanifest`）+ 应用图标（`public/icons/`）
- iOS 专属 meta 标签（主屏幕图标、独立全屏运行、应用名称「口袋记账」）
- Service Worker（`public/sw.js`）：预缓存应用外壳 + 运行时缓存静态资源，支持离线打开

注意两点：

- **离线缓存在 `http://` 下不生效**：Service Worker 仅在 HTTPS 或 localhost 下注册。局域网 IP 通过 `http://` 访问时，主屏幕安装、全屏等一切正常，只是断网后无法打开；完整离线体验需部署到 HTTPS 环境
- **dev（5173）与 preview（4173）数据相互独立**：各自使用独立的 localStorage，账目数据不互通；数据保存在手机本地，「添加到主屏幕」的 App 与 Safari 浏览器的存储也是隔离的，删除主屏幕图标会清掉该 App 内的数据
- 发版更新（HTTPS 部署后）：改代码后同步递增 `public/sw.js` 顶部的 `VERSION` 以清理旧缓存，页面导航为网络优先，联网刷新即可拿到新版
...
- **第三批收尾**：二级分类、组合付款
- ~~**PWA**：主屏幕安装、应用图标、Service Worker 离线缓存~~ ✅ 已完成
- **第四批（数据与平台）**：支付宝 / 微信 CSV 导入、多币种、WebDAV / 自建云同步、存钱计划
- **第五批（体验深化）**：记账图片与位置、年度总结报告
